...
class TestJSONOperations:
    ...
    def test_store_complex_json_result(self, test_db):
        ...
        # Verify complex JSON was stored and can be retrieved
        retrieved_result = json.loads(task.result)
        assert retrieved_result["html_report"].startswith("# Complex Report")
        assert len(retrieved_result["history"]) == 2
    ...
    def test_json_serialization_edge_cases(self, test_db):
        ...
        special_result = {
            "content": "Special chars: àáâãäåæçèéêë 中文测试 🎉",
            "symbols": "!@#$%^&*()_+-=[]{}|;':\",./<>?"
        }

        task = Task(
            id=task_id,
            prompt="JSON edge cases",
            status="done",
            result=json.dumps(special_result, ensure_ascii=False)
        )
        ...
        retrieved_result = json.loads(task.result)
        assert "中文测试" in retrieved_result["content"]
        assert "🎉" in retrieved_result["content"]

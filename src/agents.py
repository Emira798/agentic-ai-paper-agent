...
# 工具 schema（传给 API）
TOOL_SCHEMAS = [arxiv_tool_def, tavily_tool_def, wikipedia_tool_def]
...
# === Research Agent ===
def research_agent(
    prompt: str, model: str = "qwen-turbo", return_messages: bool = False
):
    print("==================================")
...
""".strip()
    ...
    try:
        # 手动工具调用循环（最多 5 轮，替代 aisuite 的 max_turns）
        all_calls = []
        content = ""
        ...
        for turn in range(5):
            ...
            for tc in msg.tool_calls:
                all_calls.append((tc.function.name, tc.function.arguments))
            ...
            for tc in msg.tool_calls:
                ...
                if fn:
                    ...
                    result_str = _json.dumps({"error": f"Unknown tool: {tc.function.name}"})
